    ...
    let prom = new Promise((resolve, reject) => {
      ...
      let req = get(requestOpts, (res) => {
        ...
        res.on('data', (chunk) => {
          ...
          if (this.options.maxSize && xml.length > this.options.maxSize ) {
            return reject(new Error("Feed exceeds allowed size"));
          } else if (xml.length > this.options.maxSize) {
            return reject(new Error(`Feed exceeds allowed size of ${this.options.maxSize / 1000000}MB`));
          }
        });
        res.on('end', () => {
