...
class Parser {
  ...
  parseItemRss(xmlItem, itemFields) {
    ...
    if (xmlItem.enclosure) {
      ...
      item.enclosures = xmlItem.enclosure.map((enc) => enc.$);
    }
    if (xmlItem['ppg:enclosureSecure']) {
      item.enclosures = item.enclosures.concat(
        xmlItem['ppg:enclosureSecure'].map((enc) => enc.$)
      );
    }
    if (xmlItem['ppg:enclosureLegacy']) {
      item.enclosures = item.enclosures.concat(
        xmlItem['ppg:enclosureSecure'].map((enc) => enc.$)
      );
    }
