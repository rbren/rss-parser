...
class Parser {
  constructor(options = {}) {
    options.xmlHeader = options.xmlHeader || false;
  ...
  parseString(xml, callback) {
    let prom = new Promise((resolve, reject) => {
      if (this.options.xmlHeader) {
        let encoding = utils.getXmlEncoding(xml);
        xml = iconv.decode(new Buffer(xml, encoding), encoding).toString("utf8");
      }
