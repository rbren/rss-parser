  ...
  parseURL(feedUrl, callback, redirectCount=0) {
    ...
    let prom = new Promise((resolve, reject) => {
      let req = get({
        ...
        let encoding;
        let originEncoding = utils.getEncodingFromContentType(res.headers['content-type']);
        const needDecode = utils.needDecodeEncoding(originEncoding);
        if (needDecode) {
          encoding = utils.binaryEncodeing;
        } else {
          encoding = utils.getSupportedEncoding(originEncoding);
        }
