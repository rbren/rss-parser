...
utils.getXmlEncoding = function (xml) {
  var firstLine = xml.split('\n')[0];
  if ( firstLine.indexOf("<?xml") !== -1 && firstLine.indexOf("encoding" !== -1)) {
    var regexEncoding=/encoding\s*=\s*((?:'|")(.*)(?:'|"))/i;
    let match = regexEncoding.exec(firstLine)
    if ( match ) {
      let encoding = utils.getEncodingAlias(match[2].toLowerCase());
      return encoding;
    }
  }
  return "utf8";
}
utils.getEncodingAlias = function(encoding) {
...
utils.getEncodingFromContentType = function(contentType) {
  ...
  return utils.getEncodingAlias(encoding);
}
